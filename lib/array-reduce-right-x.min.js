 ...
 */
"use strict";var nativeReduceRight=Array.prototype.reduceRight,rRightCoercesToObject=!1;if(nativeReduceRight)try{rRightCoercesToObject="object"==typeof nativeReduceRight.call("es5",function(_,__,___,list){return list})}catch(ignore){nativeReduceRight=null}var $reduceRight;if(nativeReduceRight&&rRightCoercesToObject)$reduceRight=function reduceRight(array,callBack){var args=[callBack];return arguments.length>2&&args.push(arguments[2]),nativeReduceRight.apply(array,args)};else{var boxedString=Object("a"),splitString="a"!==boxedString[0]||0 in boxedString==!1,toObject=_dereq_("to-object-x"),assertIsFunction=_dereq_("assert-is-function-x"),isString=_dereq_("is-string"),toLength=_dereq_("to-length-x"),errMsg="reduceRight of empty array with no initial value";$reduceRight=function reduceRight(array,callBack){var object=toObject(array);assertIsFunction(callBack);var iterable=splitString&&isString(object)?object.split(""):object,length=toLength(iterable.length);if(0===length&&2===arguments.length)throw new TypeError(errMsg);var result,i=length-1;if(arguments.length>2)result=arguments[2];else for(;;){if(i in iterable){result=iterable[i],i-=1;break}if((i-=1)<0)throw new TypeError(errMsg)}for(;i>=0;)i in iterable&&(result=callBack(result,iterable[i],i,object)),i-=1;return result}}module.exports=$reduceRight},{"assert-is-function-x":2,"is-string":15,"to-length-x":25,"to-object-x":26}],2:[function(_dereq_,module,exports){/**
